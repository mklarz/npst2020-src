...
export let windowTitle = "Ikke navngitt vindu";
export let id;
export let order;
...
let thisWindowWidth = 0;
let thisWindowHeight = 0;
let globalWindowWidth = 0;
let globalWindowHeight = 0;
$: dynamicStyle = `
		z-index: ${10 + order};
		transform: translate(${self === null || self === void 0 ? void 0 : self.pos[0]}px, ${self === null || self === void 0 ? void 0 : self.pos[1]}px);
		width: ${width}px;
		height: ${height}px;
	`;
function handleDrag(event) {
    if (!self)
        return;
    self.pos = [
        Math.max(0, Math.min(self.pos[0] + event.detail.dx, globalWindowWidth - thisWindowWidth)),
        Math.max(0, Math.min(self.pos[1] + event.detail.dy, globalWindowHeight - thisWindowHeight)),
    ];
}
function handleResize(event) {
    width = Math.max(200, Math.min(width + event.detail.dx, globalWindowWidth));
    height = Math.max(100, Math.min(height + event.detail.dy, globalWindowHeight - 80));
}
onMount(() => {
